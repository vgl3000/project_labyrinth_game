...
EVENT_PROBABILITY = 10  # 1 из 10 шанс
TRAP_DEATH_THRESHOLD = 3  # 30% шанс смерти при пустом инвентаре
...
def solve_puzzle(game_state: dict) -> None:
    """
    Обрабатывает попытку игрока решить загадку в текущей комнате.

    Если загадки нет — выводит сообщение.
    Если ответ верный — убирает загадку и выдаёт награду.
    Если ответ неверный в trap_room — срабатывает ловушка.
...
def trigger_trap(game_state: dict) -> None:
    """
    Активирует ловушку: удаляет случайный предмет из инвентаря или завершает игру.

    Если инвентарь не пуст — удаляется один случайный предмет.
    Если инвентарь пуст — с 30% шансом игра завершается.

    Args:
        game_state (dict): Словарь с состоянием игры, содержит ключи:
            - 'player_inventory' (list): список предметов игрока
            - 'steps_taken' (int): количество шагов (используется как seed)
    ...
    if inv:
        ...
        if danger_roll < TRAP_DEATH_THRESHOLD:  # 30% шанс
...
def random_event(game_state: dict) -> None:
    """
    Генерирует редкое случайное событие после перемещения игрока.

    Событие происходит с вероятностью 1 к EVENT_PROBABILITY (по умолчанию 10).
    Возможные события:
    - Находка монетки в комнате
    - Испуг (шорох), отпугиваемый мечом
